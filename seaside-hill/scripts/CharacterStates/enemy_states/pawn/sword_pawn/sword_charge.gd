...
func enter() -> void:
	...
	if parent.animated_sprite_2d.flip_h:
		parent.direction = 1
		parent.sword.position = parent.charge_right.global_position
		parent.sword.rotate(PI/2)
		
	else:
		parent.direction = -1
		parent.sword.position = parent.charge_left.global_position
		parent.sword.rotate(-PI/2)
